...
require('../util/calculate_distance.php');
...
if (!$connection) {
  ...
  error_log("getCollPlayersHistory: ".pg_get_result($connection), 3 ,"debug_file.log");
	...
	while ($rowCollHistory = pg_fetch_row($resultCollHistory)){
		...
		if ($lat != "0" &&  !in_array($playerID,$playersCollHistoryArray)){
			$allHistory[] = array("id"=>"coll","timestamp"=>$rowCollHistory[0],"collplayers"=>$playersCollHistoryString,"lng"=>$lng,"lat"=>$lat, "dist"=>$dist);
		}
	}
	...
	$queryEmoHistory = 'SELECT playername, emoname, emotimstamp, emolocation FROM (SELECT A1.name as playername, 
					...
					WHERE A1.id = A2.player_id AND A2.emotion_id = A3.id AND NOT A1.id = '.$playerID.' ORDER BY emotimstamp DESC) as Temp;';
	...
	while ($rowEmoHistory = pg_fetch_row($resultEmoHistory)){
		if ($rowEmoHistory[3] != "" && $myLat !=""){
			...
			$allHistory[] = array("id"=>"emo","player"=>$rowEmoHistory[0],"emotion"=>$rowEmoHistory[1],"timestamp"=>$rowEmoHistory[2],"lng"=>$lng,"lat"=>$lat, "dist"=>$dist);
		}
		else{
	...
	$queryHugHistory = 'SELECT sender, A2.name as reciever, time, location FROM (
				SELECT A2.name as sender, A1.reciever as reciever, A1.timestamp as time, ST_AsText(location) as location FROM players A2, players_hug_history A1 WHERE A1.sender = A2.id AND NOT A1.sender = '.$playerID.') 
	...
	while ($rowHugHistory = pg_fetch_row($resultHugHistory)){
		...
		if ($rowHugHistory[3] != "" && $myLat !=""){
			...
			$allHistory[] = array("id"=>"hug","sender"=>$rowHugHistory[0],"reciever"=>$rowHugHistory[1],"timestamp"=>$rowHugHistory[2],"lng"=>$lng,"lat"=>$lat, "dist"=>$dist);
		}else{
...
for($i = 0, $size = sizeof($allHistorySortedZero); $i < $size; ++$i){
	...
	case "emo":
	if ($allHistorySortedZero[$i]["dist"] < 500 && $emocount < 10){
		...
		$newnode->setAttribute("dist", $allHistorySortedZero[$i]["dist"]);
		$emocount++;
	}
		break;
	case "coll":
	if ($allHistorySortedZero[$i]["dist"] < 500 && $collcount < 10){
		...
		break;
	case "hug":
	if ($allHistorySortedZero[$i]["dist"] < 500 && $hugcount < 10){
