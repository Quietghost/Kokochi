...
if (!$connection) {
  ...
  error_log("getLastEmotionStatementByPlayer: ".pg_get_result($connection), 3 ,"debug_file.log");
	...
	//$query = 'SELECT count(id) FROM players_coll_history WHERE coll_players LIKE \'%_'.$playerID.'%\' OR \'%'.$playerID.'_%\';';
	...
	// Iterate through the rows, printing XML nodes for each
		...
		$newnode->setAttribute("collaborations", $row['count']);
	...
	while ($row = pg_fetch_row($result)){
		...
		$lastEmotionName = "";
		if ($row[2] != "0"){
			$queryPlayerEmotion = 'SELECT name FROM '.$emoTable.' WHERE id='.$row[2];
