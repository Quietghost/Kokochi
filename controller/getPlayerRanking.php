...
if (!$connection) {
  ...
  //DEBUG
  error_log("getPlayerRanking: ".pg_get_result($connection), 3 ,"debug_file.log");
}

// Select all the rows in the markers table
$query = 'SELECT id, name, flower_points FROM "players" ORDER BY flower_points DESC';
...
if (!$result) {
  ...
  //DEBUG
  error_log("getPlayerRanking: ".pg_last_error(), 3 ,"debug_file.log");
  exit;
}
...
while ($row = pg_fetch_row($result)){
 ...
 // ADD TO XML DOCUMENT NODE
  ...
  $newnode->setAttribute("flower_points", $row[2]);
